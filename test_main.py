...
class TimerWidget(QtWidgets.QFrame):
    ...
    def init_ui(self):
        ...
        self.cancel_button.setEnabled(False)  # 初始狀態為 disabled - 內建的視覺回饋機制
    ...
    def start_timer(self):
        ...
        if self.remaining_time <= 0:
            QtWidgets.QMessageBox.critical(self, "錯誤", "請輸入正的分鐘數")
            return
        self.resume_timer() # 啟動計時器

    # 暫停計時器
    def pause_timer(self):
        self.is_running = False
        self.start_button.setEnabled(True)  # 按鈕可以按
        self.cancel_button.setEnabled(True)  # 按鈕不能按
        self.timer.stop()
        self.start_button.setText("繼續")  # 改文字
    ...
    def resume_timer(self):
        ...
        self.cancel_button.setEnabled(True)
        self.start_button.setText("暫停")  # 改文字

        # 重新計算到期時間 (因為暫停期間時間流逝了，所以要用現在時間 + 剩餘時間 重算)
        self.end_time = datetime.now() + timedelta(seconds=self.remaining_time)
        self.update_end_time_label()
    ...
    def cancel_timer(self):
        ...
        self.cancel_flash()
        # 重新啟動計時器
        self.start_button.setText("開始") # 確保按鈕文字回到 "開始"
...
class CountdownApp(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("GleamTimerV7-By Peggy")  # 設置窗口標題
